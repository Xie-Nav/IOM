...
class NavigationAgent(ThorAgent):
    ...
    def __init__(self, create_model, args, rank, scenes, targets, gpu_id):
        ...
        self.obstacle_map_dim = args.obstacle_map_dim

    def eval_at_state(self, model_options, last_action_success):
        ...
        model_input.last_action_success = last_action_success
        model_input.imp_obstacle_map = self.imp_obstacle_map 
        model_input.obstacle_index = self.obstacle_index   
    ...
    def reset_hidden(self):
        ...
        self.imp_obstacle_map = gpuify(
            torch.zeros((1, self.obstacle_map_dim)), self.gpu_id
        )
        index = gpuify(
            torch.ones(1, 1) * -1, self.gpu_id
        )
        obstacle_index = list()
