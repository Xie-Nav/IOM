...
class BasicEpisode(Episode):
    ...
    def step(self, action_as_int, start_coord, obstacle_index, imp_obstacle_map):
        ...
        reward, terminal, action_was_successful = self.judge(action, start_coord, obstacle_index, imp_obstacle_map)
        return reward, terminal, action_was_successful

    def judge(self, action, start_coord, obstacle_index, imp_obstacle_map):
        """ Judge the last event. """
        reward = STEP_PENALTY

        # if imp_obstacle_map.size()[0]>=2:
        #     index2 = torch.where((imp_obstacle_map[-2,:]) == -1)[0]
        #     if not (index2.size(0) == 0):
        #         reward = reward + 0.02
        agent_x = np.array(self.environment.controller.state.x)
        agent_y = np.array(self.environment.controller.state.z)
        # 如果到过的地点以前就到达过，那么就需要给出惩罚
        if self.environment.last_action_success and action['action'] == 'MoveAhead':
            cur_xy = torch.zeros(1, 2).to(imp_obstacle_map.device)
            cur_xy[:, 0] = agent_x - start_coord['x']; cur_xy[:, 1] = agent_y - start_coord['y']
            index = torch.where(((imp_obstacle_map[:, -2 :] - cur_xy).pow(2).sum(1)) == 0)[0]
            if index.size(0) != 0:
                reward = reward - 0.01            
            
        # 避开障碍物，给出一个奖励
        if len(obstacle_index)==1:
            index = torch.where((imp_obstacle_map[obstacle_index[-1].type(torch.long),:]) == -1)[0]
        else:
            index = torch.where((imp_obstacle_map[obstacle_index[-2].type(torch.long),:]) == -1)[0]  
        if (not (index.size(0) == 0)) and self.environment.last_action_success and action['action']=='MoveAhead' :
            reward = reward + 0.02
        
        # 发生碰撞给出惩罚
        if (not self.environment.last_action_success) and action['action']=='MoveAhead':
            reward = reward - 0.01
